...
/*Stage.prototype.deferQ = function() {
	var self = this;
	var defer = q.defer();
	debugger;
	defer.resolve(function(err, ctx) {
		if (!err) {
			self.execute(ctx, resPromise.makeNodeResolver());
		} else {
			defer.reject(err);
		}
	});
...
// продумать, скорее контекст чем stage должен биндиться...
Stage.prototype.promiseQ = function(ctx) {
	// so far promis has memory and it is not reenterable, we need to fix context.
	var self = this;
	var resPromise = q.defer(); //mpromise.deferred();
	process.nextTick(function() {
		self.execute(ctx, resPromise.makeNodeResolver());
	});
	return resPromise.promise;
